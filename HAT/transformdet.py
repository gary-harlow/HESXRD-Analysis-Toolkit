...
def dector_frame_to_hkl_frame(window,binning,twodetectors):
    ...
    if  acceleration==2:
        ...
        def detector_to_hkl_kernel(h_glob,k_glob,l_glob,hk_glob,c_glob):
            ...
            if row < hk_glob.shape[0] and col < hk_glob.shape[1]:
                ...
                sign = math.copysign(1,psi)    
                h_glob[row,col] = qx
                k_glob[row,col] = qy
                l_glob[row,col] = qz
                c_glob[row,col] = abs(C)
                hk_glob[row,col] = sign*math.sqrt(qx**2 + (r*qy)**2)      
    ...
    def _dector_frame_to_hkl_frame():
            ...
            for row in range(pixel_count_y):    
                for col in range(pixel_count_x):
                    ...
                    l_img[row][col] = qz
                    c_img[row][col] = abs(C)
                    hk_img[row][col] = sign*math.sqrt(qx**2 + (r*qy)**2)      
